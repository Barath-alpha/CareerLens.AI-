...
def create_and_train_model():
    ...
    for _ in range(n_samples):
        ...
        # Placement label based on weighted score
        score = (
            cgpa * 0.25 +
            (tenth / 10) * 0.05 +
            (twelfth / 10) * 0.05 +
    ...
    print(f"[ML] Training accuracy: {model.score(X, y):.4f}")
    return model

def load_model():
    """Load the model, training if not found."""
...
def predict_placement(features: dict) -> dict:
    ...
    Returns: { probability, result, confidence, placement_score, strengths, weaknesses, suggestions }
    """
    model = get_model()
    ...
    feature_vector = np.array([[
        safe_get('cgpa', 7.0),
        safe_get('tenth_percent', 75.0),
        safe_get('twelfth_percent', 75.0),
        safe_get('current_sem_percent', 75.0),
        safe_get('communication_score', 5.0),
        safe_get('technical_score', 5.0),
        safe_get('programming_score', 5.0),
        safe_get('aptitude_score', 5.0),
        safe_get('logical_reasoning', 5.0),
        safe_get('problem_solving', 5.0),
        safe_get('leadership_score', 5.0),
        safe_get('soft_skills', 5.0),
        safe_get('interview_score', 5.0),
        safe_get('projects_count', 0),
        safe_get('internships_count', 0),
        safe_get('hackathons_count', 0),
        safe_get('certificates_count', 0),
        safe_get('leetcode_solved', 0),
        safe_get('github_activity', 0.0),
        safe_get('attendance', 75.0),
        safe_get('backlogs', 0),
        safe_get('coding_contest_rating', 0),
    ]])
